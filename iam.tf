resource "aws_iam_role" "ec2_runner" {
  name               = "gitlab-runner"
  assume_role_policy = file("iam/services/ec2.json")
}

data "template_file" "ec2_runner_policy" {
  template = file("iam/ec2_runner_policy.json")
  vars = {
...
data "template_file" "lambda_gitlab_metric_policy" {
  template = file("iam/lambda_gitlab_metric_policy.json")
  vars = {
...
resource "aws_iam_role" "lambda_gitlab_metric" {
  ...
  assume_role_policy = file("iam/services/lambda.json")
}
